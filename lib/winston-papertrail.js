 ...
 *
 * @param {Boolean}     [options.humanReadableUnhandledException]   pretty-print unhandled exceptions
 *
...
var Papertrail = exports.Papertrail = function (options) {
    ...
    // Pretty-print unhandled exceptions
    self.humanReadableUnhandledException = options.humanReadableUnhandledException || false;
...
Papertrail.prototype.log = function (level, msg, meta, callback) {
    ...
    if (meta) {
        if (typeof meta !== 'object') {
            ...
            if (self.humanReadableUnhandledException
                ...
                delete meta.stack;
                delete meta.trace;
                output += ' ' + util.inspect(meta, false, null, self.colorize);

                if (stack) {
                    output += '\n' + stack.join('\n');
                }
            }
            else if (this.inlineMeta) {
